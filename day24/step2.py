...
for _ in range(15 * (height + width)):
    bpos = set()
    for b in blizzards:
        bpos.add((b.x, b.y))
...
def test(x, y, g, t, s, n):
    if t >= 15 * (height + width)-1:
        d[(x, y, t, n)] = float('inf')
        return
    nn = n
    if (x, y) == end:
        if n == 0:
            ...
            d[(x, y, t, n)] = t
            return
    if (x, y) == start and n == 1:
        nn = 2
    if (x, y) in s and s[(x, y)] >= 10:
        d[(x, y, t, n)] = float('inf')
        return
    else:
        ns = s.copy()
    ...
    res = float('inf')
    for (nx, ny) in moves:
        if (x+nx, y+ny, t+1, nn) in d:
            res = min(res, d[(x+nx, y+ny, t+1, nn)])
        elif 0<=x+nx<=width and 0<=y+ny<=height and (x+nx, y+ny) not in g[t+1]:
            test(x+nx, y+ny, g, t+1, ns, nn)
            res = min(res, d[(x+nx, y+ny, t+1, nn)])
    d[(x, y, t, n)] = res
    return
